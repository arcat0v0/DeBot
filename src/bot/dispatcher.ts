...
const CREDENTIAL_HINTS: Record<ProviderId, string> = {
  ...
  azure: [
    ...
    "  --scopes /subscriptions/订阅ID/resourceGroups/资源组</code>",
    "输出里：appId = clientId，password = clientSecret，tenant = tenantId。",
    "订阅ID 可在 门户 → 「订阅」 中查看。",
    "",
    "然后发送：",
    "<code>tenantId clientId clientSecret 订阅ID 资源组</code>",
    "（也可以直接发送 JSON）",
  ].join("\n"),
  gcp: [
